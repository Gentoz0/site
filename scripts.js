window.addEventListener('DOMContentLoaded', (event) => { //обработчик события DOMContentLoaded позволяет выполнить код после загрузки и инициализации DOM, а объект event предоставляет информацию о событии, которое произошло, в обработчике этого события.
    const fileInput = document.getElementById('file-input'); // получение ссылок
    const uploadButton = document.getElementById('upload-button');
    const dataPreview = document.getElementById('data-preview');

    uploadButton.addEventListener('click', () => {
        fileInput.click(); // При клике на эту кнопку будет вызвано действие выбора файла.
    });
    ...
    fileInput.addEventListener('change', () => {
        ...
        if (file) {
            ...
            reader.readAsText(file); // Запускаем процесс чтения содержимого файла
        }
    });

    function displayCSV(csvContent) { // Разбиение таблицы на строки чтобы было удобно смотреть
...
function openTab(tabName) {
    ...
    for (i = 0; i < tabcontent.length; i++) {
        tabcontent[i].style.display = "none"; // скрытие
    }
...
document.querySelectorAll('nav ul li a').forEach(link => {
    link.addEventListener('click', function(e) {
        ...
        var tabName = this.getAttribute('href').substring(1); // Получает атрибут href текущей ссылки, который содержит идентификатор вкладки, на которую она ссылается.
...
function addOrUpdateStudent() { //добавление нового ученика
...
function calculateAverage(grades) { // принимаем массив оценок
    const sum = grades.reduce((a, b) => a + b, 0); //  просто складывает элементы массива a и b.
    return (sum / grades.length) || 0;
}

// Функция для вычисления медианы
function calculateMedian(grades) {
    const sortedGrades = [...grades].sort((a, b) => a - b); // сортировка в копии массива
    const mid = Math.floor(sortedGrades.length / 2); // Вычисляется индекс среднего элемента массива.
    return sortedGrades.length % 2 !== 0
        ? sortedGrades[mid]
        : (sortedGrades[mid - 1] + sortedGrades[mid]) / 2; // Возвращается либо значение элемента sortedGrades[mid], если количество оценок в массиве grades нечетное, либо среднее арифметическое двух средних элементов массива, если количество оценок четное.
}

// Функция для подсчета учеников, получивших каждую оценку
function countGrades(grades) {
    const counts = { '2': 0, '3': 0, '4': 0, '5': 0 }; //0 это счетчик оценок
    grades.forEach(grade => {
        counts[grade]++;
    }); // считает сколько каких оценок
    return counts;
}
function generateStatistics() {
    // Предполагаем, что таблица со студентами имеет id 'students-table'
    const studentsTable = document.getElementById('students-table'); // получаем ссылку на таблицу
    if (!studentsTable) {
    ...
    for (let row of studentsTable.rows) { //перебираем все строки
        ...
        const classInfo = row.cells[1].textContent; // Получение информации о классе из второй ячейки текущей строки.
        const grades = Array.from(row.cells).slice(2, -1).map(cell => parseInt(cell.textContent, 10)); // Извлечение оценок из оставшихся ячеек текущей строки.

        // Инициализация статистики для класса, если ещё не существует
        if (!statistics[classInfo]) {
            statistics[classInfo] = grades.map(() => ({ //создание массива map
                total: [],
                average: 0,
                median: 0,
        ...
        grades.forEach((grade, i) => { // перебор оценок 
            if (grade) {
                statistics[classInfo][i].total.push(grade); // добавляется в массив
            }
        });
    }
    ...
    for (let studentName of Object.keys(studentStatistics)) { // перебор всех ключей в объекте studentStatistics.
...
function createClassChart(statistics) {
    const classData = {
        labels: Object.keys(statistics), // создается объект classData, который будет содержать данные для графика.
        datasets: []
    };
    ...
    subjects.forEach((subject, index) => {
        const dataset = {
            label: subject, // метка для предмета
            data: [], // массив для данного предмета
            backgroundColor: getRandomColor(), // Функция для получения случайного цвета
            borderWidth: 1
        };

        Object.values(statistics).forEach(classStats => { //для каждого предмета добавляется средняя оценка
    ...
    const ctx = document.getElementById('class-chart').getContext('2d'); // получавем контекст для рисования

    // Создаем график
    new Chart(ctx, {
        type: 'bar', // тип диаграммы
        data: classData, // данные для графика
        options: {
            scales: {
                y: {
                    beginAtZero: true
...
function displayStatistics(statistics, studentStatistics) {
    const statsSection = document.getElementById('table-stats'); // получение ссылки на размещение
    let html = '<table class="table-stat">'; // будет содержать код для таблицы
    html += '<thead>';
    html += '<tr><th>Класс</th><th>Предмет</th><th>Средняя оценка</th><th>Медиана</th>'; // заголовки 
    ...
    for (let classInfo of Object.keys(statistics)) {
        statistics[classInfo].forEach((subjectStats, index) => {
            html += `<tr>`;
            html += `<td>${classInfo}</td>`; //добавления ячеек
...
function tableToDataString(table, separator = ',', lineEnding = '\n', excludeLastColumn = true) {
    let dataString = ''; // для хранения строк
    for (const row of table.rows) { //перебираем строки
        let rowData = []; // массив для значений ячеек строк
        const cellsLength = excludeLastColumn ? row.cells.length - 1 : row.cells.length;
        for (let i = 0; i < cellsLength; i++) { //перебор ячеек
            let cellText = row.cells[i].textContent; //индекс ячейки с текстом
            // Удаление переводов строки и замена кавычек для корректного CSV формата
            cellText = cellText.replace(/(\r\n|\n|\r)/gm, " ").replace(/"/g, '""');  //Удаляем переводы строки и заменяем двойные кавычки на двойные двойные кавычки для корректного формата CSV.
            rowData.push(`${cellText}`); //Добавляем текст ячейки в массив rowData.
        }
        dataString += rowData.join(separator) + lineEnding; //Объединяем значения массива rowData в одну строку с разделителем separator, добавляем символ конца строки lineEnding и добавляем к строке dataString.
    }
    return dataString;
}
...
function downloadFile(filename, content, mimeType) {
    ...
    const blob = new Blob([BOM + content], { type: mimeType }); //Создается объект Blob, который содержит данные для загрузки.
    const url = URL.createObjectURL(blob); //Создается URL-адрес объекта Blob с помощью метода
    const downloadLink = document.createElement("a"); //создается элемент <a>, который будет использоваться для загрузки файла.
    downloadLink.href = url; //  Устанавливается ссылка загрузки на созданный URL-адрес.
    downloadLink.download = filename; //Устанавливается имя файла для загрузки.
    document.body.appendChild(downloadLink); // Элемент <a> добавляется в тело документа для корректного его отображения и функционирования.
    downloadLink.click(); // Происходит "клик" по элементу <a>, что инициирует загрузку файла.
    document.body.removeChild(downloadLink); // После завершения загрузки элемент <a> удаляется из тела документа.
    URL.revokeObjectURL(url); //URL-адрес объекта Blob освобождается с помощью метода revokeObjectURL, чтобы избежать утечек памяти.
}

// Функция для скачивания таблицы в различных форматах
function downloadTableData() { //Это функция, которая будет вызываться при нажатии на кнопку загрузки данных.
    const table = document.getElementById('students-table'); //олучение ссылки на элемент таблицы по его id
    if (table) {
...
document.getElementById('download-data-button').addEventListener('click', downloadTableData); //когда кнопка будет нажата, будет вызываться функция downloadTableData, 
